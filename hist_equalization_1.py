...
def histogram_equalization_luv_domain(input_image, tmp, W_1, H_1, W_2, H_2):
    """
    ...
    def calc_histogram(L):
        hist_i[math.floor(L)] += 1

    def calc_f_i(hist):
        f_i = [0] * k
        f_i[0] = hist[0]
        for i in range(1, len(hist)):
            f_i[i] = f_i[i - 1] + hist[i]
        return f_i

    def calc_equalized_histogram():
        equalized_hist = [0] * k
        f_i = calc_f_i(hist_i)
        n = f_i[-1]
        equalized_hist[0] = math.floor(0.5 * f_i[0] * k / n)
        for i in range(1, len(f_i)):
            equalized_hist[i] = math.floor(0.5 * k * (f_i[i] + f_i[i - 1]) / n)
            if equalized_hist[i] == k:
                equalized_hist[i] = k - 1
    ...
    equalized_histogram = calc_equalized_histogram()

    def histogram_equalization(b, g, r):
        L, u, v = srgb_to_luv(b, g, r)
        new_l = equalized_histogram[math.floor(L)]
    ...
    for i in range(H_1, H_2 + 1):
        for j in range(W_1, W_2 + 1):
            b, g, r = input_image[i, j]
            b, g, r = histogram_equalization(b, g, r)
            tmp[i, j] = b, g, r
...
def main():
    ...
    histogram_equalization_luv_domain(input_image, tmp, W_1, H_1, W_2, H_2)
